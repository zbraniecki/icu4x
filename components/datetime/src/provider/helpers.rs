...
impl<'data> DateTimePatterns<'data> for provider::gregory::DatePatternsV1<'data> {
    ...
    fn get_pattern_for_datetime_length(
        ...
        let gp = crate::pattern::generic::GenericPattern { items: s.0.clone() };
        Ok(gp.combined(date, time)?)
    }
    ...
    fn get_pattern_for_time_length(
        ...
        let mut pattern = Pattern::from(match length {
        ...
        }) = preferences
        {
            ...
            let mut pattern: alloc::vec::Vec<_> =
                if let zerovec::ZeroVec::Owned(ref zv) = pattern.items {
