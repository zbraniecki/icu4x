...
use icu_provider::{DataPayload, DataProvider, DataRequest, ResourceOptions, ResourcePath};
...
use crate::{
    ...
    provider::{
        self,
        gregory::patterns::{PatternFromPatternsV1Marker, PatternV1},
            ...
            .map_project_with_capture(options, |data, options, _| {
                PatternV1(
                    data.get_pattern_for_options(&options)
                        ...
                        .expect("Failed in some other way")
                        .items,
                )
            });
