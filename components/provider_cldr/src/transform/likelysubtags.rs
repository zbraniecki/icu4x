...
impl From<&cldr_json::Resource> for LikelySubtagsV1 {
    fn from(other: &cldr_json::Resource) -> Self {
        ...
        let mut language_script: Vec<(u64, u32, LanguageIdentifier)> = Vec::new();
        let mut language_region: Vec<(u64, u32, LanguageIdentifier)> = Vec::new();
        let mut language: Vec<(u64, LanguageIdentifier)> = Vec::new();
