...
pub type MF2Function<'s> =
    Box<dyn Fn(&dyn VariableType, &MessageFormat) -> Vec<Box<dyn MessagePart<'s> + 's>>>;
...
impl<'b> MessageFormat<'b> {
    ...
    pub fn format_to_string<'m, 'mv, 'varsv, 'varsm, 'mf, 'mpv, MV>(
        &'mf self,
        msg: &'m ast::Message<MV>,
        variables: Option<&'varsm Variables<'varsv>>,
    ) -> Cow<'mpv, str>
    where
        MV: Slice<'mv>,
        'mv: 'mpv,
    ...
    pub fn format_to_parts<'m, 'mv, 'varsv, 'varsm, 'mf, 'mpv, MV, MPV>(
        &self,
        msg: &ast::Message<MV>,
        variables: Option<&'varsm Variables<'varsv>>,
    ...
    pub fn format_from_source<'m, 'mv, 'varsv, 'varsm, 'mf, 'mpv, MV>(
        &'mf self,
        source: MV,
        variables: Option<&'varsm Variables<'varsv>>,
    ) -> Cow<'mpv, str>
...
mod test {
    ...
    fn sanity_check() {
        ...
        let result = mf.format_from_source::<&str>("{Hello World}", None);
    ...
    fn variable_check() {
        ...
        variables.insert("name".to_string(), String::from("John"));
    ...
    fn function_check() {
        ...
        mf.functions.insert(
            "number".to_string(),
            Box::new(
                |input: &dyn VariableType, mf: &MessageFormat| -> Vec<Box<dyn MessagePart>> {
                    // let s = input.as_cow();
        ...
        let mut variables = Variables::new();
        variables.insert("emailCount".to_string(), "5.0".to_string());
