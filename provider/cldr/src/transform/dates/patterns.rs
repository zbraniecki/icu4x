...
impl From<&cldr_json::Dates> for gregory::DatePatternsV1 {
    fn from(other: &cldr_json::Dates) -> Self {
        ...
        let pattern_full = pattern::reference::Pattern::from_bytes(pattern_str_full)
            .expect("Failed to create a full Pattern from bytes.");
        let pattern_long = pattern::reference::Pattern::from_bytes(pattern_str_long)
            .expect("Failed to create a long Pattern from bytes.");
        let pattern_medium = pattern::reference::Pattern::from_bytes(pattern_str_medium)
            .expect("Failed to create a medium Pattern from bytes.");
        let pattern_short = pattern::reference::Pattern::from_bytes(pattern_str_short)
        ...
        let (time_h11_h12, time_h23_h24) = {
            ...
            let alt_time = gregory::patterns::LengthPatternsV1 {
                full: alt_hour_cycle.apply_on_pattern(
                ...
                .into(),
                long: alt_hour_cycle.apply_on_pattern(
                    &date_time_formats_v1,
                    pattern_str_long,
                    pattern_long,
                )
                ...
                .into(),
                medium: alt_hour_cycle.apply_on_pattern(
                    &date_time_formats_v1,
                    pattern_str_medium,
                    pattern_medium,
                )
                ...
                .into(),
                short: alt_hour_cycle.apply_on_pattern(
                    &date_time_formats_v1,
                    pattern_str_short,
                    pattern_short,
                )
